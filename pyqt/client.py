            ...
            if not obj_tcp_queue.empty():
                ...
                for obj_data in obj_tcp_data[1]:
                    ...
                    if class_name != "unknown":
                        cv2.rectangle(mask, (x1, y1), (x2, y2), color[class_name], 4)
                        cv2.putText(mask, label, (x1, y1 - 10),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, color[class_name], 4)
...
def draw_lane_contours(frame, mask, lane_color_map):
    overlay = frame.copy()
    num_classes = len(lane_color_map)
    for class_id in range(1, num_classes):
        binary = (mask == class_id).astype(np.uint8) * 255
        contours, _ = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        color = lane_color_map[class_id]
        cv2.drawContours(overlay, contours, -1, color, thickness=2)
    return overlay

def draw_object_contours(frame, mask, obj_color_map):
    # 컬러 마스크를 적용할 때, addWeighted로 두 이미지를 합성하는 방식
    # 프레임에 색상 마스크를 합성
...
class VideoUpdateThread(QThread):
    ...
    def __init__(self, label_width, label_height, parent=None):
        ...
        self.lane_color = {
            0: [0, 0, 0],          # background
            1: [0, 0, 255],        # red (ok)
            2: [0, 255, 255],      # yellow (ok)
            3: [0, 255, 0],        # green (ok)
            4: [255, 100, 0],      # brighter orange
            5: [255, 0, 255],      # magenta (ok)
        }
    ...
    def update_msg(self, msg, cls_id, obj_time, obj_id, position=None, lane_mask = None):
        ...
        if obj_id == 3:
            if position < 30:
                self.alert_message.emit("사람이 있습니다. 정지.")
        ...
        # 차량이 너무 가까우면 강제 불가능 (예: vehicle_distance 변수가 있다면)
        if hasattr(self, "position") and position < 15:
            self.lane_message.emit("차선 변경 불가능")
            if obj_id is not None:
                db_queue.put(("action_log", {
                    "object_id": obj_id,
        ...
        # 정지 조건: 빨간불 + 정지선 감지 + 가까운 거리
        if msg[2] == 1:
            self.lane_dist = estimate_lane_distance(lane_mask, 0.1)
            # print("lane dist : ", self.lane_dist)
            if self.lane_dist < 6.0:  # 9 = veh_stop    
                self.state_message.emit("정지")
                print("lane dist : ", self.lane_dist)
                if obj_id is not None:
                    db_queue.put(("action_log", {
        ...
        if msg[3] == 1:
            if cls_id == 3:
                ...
                if self.obj_dist is not None and self.obj_dist < 40:
                    ...
                    self.alert_message.emit("")
            # elif cls_id != 3:
                # self.alert_message.emit("장애물 주의")
        else:
            self.alert_message.emit("")
        ...
        if cls_id == 1:
            self.obj_dist = position
            if self.obj_dist is not None and self.obj_dist < 15:
                self.obj_message.emit("어린이 보호구역 - 주의")
            ...
            self.obj_dist = position
            if self.obj_dist is not None and self.obj_dist < 15:
                self.obj_message.emit("공사장 근처 - 주의")
        ...
        while self.running:
            ...
            for uuid in ready_uuids:
                ...
                if self.lane_mask and self.obj_mask:
                    ...
                    frame = draw_lane_contours(frame, lane, self.lane_color)
                    # frame = cv2.addWeighted(frame, 0.8, obj_vis, 0.2, 0.0)
                    frame = draw_object_contours(frame, obj_vis, self.obj_color)
                elif self.lane_mask:
